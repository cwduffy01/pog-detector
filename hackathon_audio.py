...
faceCascade = cv2.CascadeClassifier(r'C:\Users\gosee\Desktop\pog-detector\haarcascade_frontalface_default.xml')
...
def record():
    ...
    while(True):
        ...
        if(readyToRecord and framesAfterPog < minFPS*5 - 10):
            framesAfterPog += 1
        #if it has been 5 seconds since the intital pog, it saves the video
        elif(readyToRecord):
            readyToRecord = False
            framesAfterPog = 0
            vid = cv2.VideoWriter("output.mp4", fourcc, minFPS, (SCREEN_SIZE))
            for frame in temp:
                vid.write(frame)
            ...
            video2.write_videofile("output2.mp4", fps=minFPS, codec="libx264") # mpeg4
